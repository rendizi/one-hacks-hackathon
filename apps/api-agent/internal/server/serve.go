...
import (
	...
	"github.com/rendizi/one-hacks-hackathon-agent/config"
	"github.com/rendizi/one-hacks-hackathon-agent/grpc"
	inst2 "github.com/rendizi/one-hacks-hackathon-agent/internal/inst"
	"github.com/rendizi/one-hacks-hackathon-agent/internal/redis"
	"github.com/rendizi/one-hacks-hackathon-agent/internal/services/gemini"
	"github.com/rendizi/one-hacks-hackathon-agent/internal/services/openai"
	"github.com/rendizi/one-hacks-hackathon-agent/internal/services/shotstack"
	"github.com/rendizi/one-hacks-hackathon-agent/pkg/logger"
	"go.uber.org/zap"
	"log"
	"time"
)
...
func (s *Server) SummarizeStories(req *grpc.SummarizeStoriesRequest, stream grpc.StoriesSummarizer_SummarizeStoriesServer) error {
	...
	var data string
	medias := make([]shotstack.LikeAsset, 0)
	var used float32
	used = 0
		...
		for _, story := range storiess.Reel.Items {
			...
			for _, media := range story.Videos {
				...
				if err != nil {
					...
					if addIt {
						var tempAsset shotstack.LikeAsset
